 ...
 *
 * The grid is constrained to a fixed height of [max_size] to avoid occupying unlimited space.
...
fun DeviceList(
    ...
    LazyVerticalGrid(
        columns = GridCells.Fixed(1), // Single column grid (one device per row)
        modifier = Modifier
            .fillMaxWidth()
            .height(max_size)
    ) {
        items(devices) { device ->
            DeviceCard(device, onClick = { /* TODO: Handle device click */ })
        }
    }
}
