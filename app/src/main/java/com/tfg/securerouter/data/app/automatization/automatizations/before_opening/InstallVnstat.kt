...
class InstallVnstat(
    ...
    private suspend fun isVnstatInstalled(): Int {
        val res = shLastLine("command -v vnstat >/dev/null 2>&1 && echo 1 || echo 0")
        Log.d("InstallVnstat", "isVnstatInstalled(last)='$res'")
        return if (res == "1") -1 else 1
    }

    override suspend fun shouldRun(router: RouterInfo?): Int {
        return isVnstatInstalled()
    }
    ...
    override suspend fun execute(): Boolean {
        ...
        when (pm) {
            "opkg" -> {
                sh("(opkg update >/dev/null 2>&1 || true; opkg install vnstat >/dev/null 2>&1 || true) >/dev/null 2>&1 || true")
            }
            "apk" -> {
                sh("(apk add --no-cache vnstat >/dev/null 2>&1 || true) >/dev/null 2>&1 || true")
            }
            else -> return false
        }

        return isVnstatInstalled() == -1
    }
}
