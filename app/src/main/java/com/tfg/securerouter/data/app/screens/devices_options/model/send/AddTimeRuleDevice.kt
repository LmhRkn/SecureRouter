...
object AddTimeRuleDevice {
    fun addTimeRuleDevice(deviceTimesRule: DeviceTimesRuleState) {
        ...
        val commandDifferentDays = """
            uci add firewall rule
            uci add firewall rule
            uci set firewall.@rule[-2].name="time_mac_${deviceTimesRule.mac}_${timeToName(deviceTimesRule.start, "23:59:59")}"
            uci set firewall.@rule[-2].src="lan"
            uci set firewall.@rule[-2].src_mac="${deviceTimesRule.mac}"
            uci set firewall.@rule[-2].dest="wan"
            uci set firewall.@rule[-2].start_time="${deviceTimesRule.start}"
            uci set firewall.@rule[-2].stop_time="23:59:59"
            uci set firewall.@rule[-2].weekdays="${parseStringToDays(deviceTimesRule.days)}"
